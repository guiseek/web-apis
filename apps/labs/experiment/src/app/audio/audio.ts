import { TimelineDataSeries, TimelineGraphView } from '@web-apis/util-data';
import { query } from '@web-apis/ui-web';
...
const audio2 = query('audio#audio2');
const callButton = query('button#callButton');
const hangupButton = query('button#hangupButton');
const codecSelector = query('select#codec');
...
let bitrateGraph: TimelineGraphView;
let bitrateSeries: TimelineDataSeries;
let targetBitrateSeries: TimelineDataSeries;
let headerrateSeries: TimelineDataSeries;

let packetGraph: TimelineGraphView;
let packetSeries: TimelineDataSeries;

let lastResult: RTCStatsReport;

interface VoiceActivityDetection extends RTCOfferOptions {
  voiceActivityDetection: boolean;
}

const offerOptions: VoiceActivityDetection = {
...
let audioLevelGraph: TimelineGraphView;
let audioLevelSeries: TimelineDataSeries;
...
const codecPreferences = query('select#codecPreferences')!;
...
async function setPtime(ptime: number) {
  if (!pc1) return;
  const offer = await pc1.createOffer();
  await pc1.setLocalDescription(offer);
  const desc = new SessionDescription(pc1.remoteDescription!);
...
function gotStream(stream: MediaStream) {
  ...
  pc1!
    ...
    .then(
      (sdp) => gotDescription1(new SessionDescription(sdp)),
...
function gotDescription1(desc: SessionDescription) {
  ...
  pc1!.setLocalDescription(desc).then(() => {
    ...
    pc2!.setRemoteDescription(desc).then(() => {
      return pc2!
        .createAnswer()
        .then(
          (sdp) => gotDescription2(new SessionDescription(sdp)),
          onCreateSessionDescriptionError
        );
...
function gotDescription2(desc: SessionDescription) {
...
function forceChosenAudioCodec(sdp: string) {
  ...
  sender.getStats().then((res) => {
    res.forEach((report) => {
      ...
      if (report.type === 'outbound-rtp') {
        ...
        if (lastResult && (lastResult as any).has(report.id)) {
          const deltaT =
            (now - (lastResult as any).get(report.id).timestamp) / 1000;
          // calculate bitrate
          const bitrate =
            (8 * (bytes - (lastResult as any).get(report.id).bytesSent)) /
            deltaT;
          const headerrate =
            (8 *
              (headerBytes -
                (lastResult as any).get(report.id).headerBytesSent)) /
            deltaT;
          ...
          packetSeries.addPoint(
            now,
            (packets - (lastResult as any).get(report.id).packetsSent) / deltaT
          );
...
if (
  ...
  let lastTime: number;
  const getAudioLevel = (timestamp: number) => {
