...
export class TimelineDataSeries {
  ...
  cacheStartTime_: Date | number | null = null;
  cacheStepSize_ = 0;
  cacheValues_: number[] = [];
  /**
   * @constructor
   ...
   */
  addPoint(timeTicks: number, value: number) {
...
export class DataPoint {
  constructor(readonly time: Date | number, readonly value: number) {}
}
...
export class TimelineGraphView {
  ...
  static MAX_VERTICAL_LABELS = 6;

  // Vertical spacing between labels and between the graph and labels.
  static LABEL_VERTICAL_SPACING = 4;
  // Horizontal spacing between vertically placed labels and the edges of the
  // graph.
  static LABEL_HORIZONTAL_SPACING = 3;
  ...
  static Y_AXIS_TICK_LENGTH = 10;

  static GRID_COLOR = '#CCC';
  static TEXT_COLOR = '#000';
  static BACKGROUND_COLOR = '#FFF';

  static MAX_DECIMAL_PRECISION = 3;

  scrollbar_ = { position_: 0, range_: 0 };
  graphDiv_: HTMLDivElement;
  ...
  graph_: Graph | null = null;

  // Horizontal scale factor, in terms of milliseconds per pixel.
  scale_ = 1000;

  repaintTimerRunning_ = false;

  constructor(divId: string, canvasId: string) {
    this.graphDiv_ = document.querySelector<HTMLDivElement>(`#${divId}`)!;
    this.canvas_ = document.querySelector<HTMLCanvasElement>(`#${canvasId}`)!;
  ...
  repaint() {
    ...
    // Clear the canvas.
    context.fillStyle = TimelineGraphView.BACKGROUND_COLOR;
    ...
    height -= fontHeight + TimelineGraphView.LABEL_VERTICAL_SPACING;
    this.drawTimeLabels(context, width, height, textHeight, visibleStartTime);

    // Draw outline of the main graph area.
    context.strokeStyle = TimelineGraphView.GRID_COLOR;
  ...
  drawTimeLabels(
    ...
    context.fillStyle = TimelineGraphView.TEXT_COLOR;
    context.strokeStyle = TimelineGraphView.GRID_COLOR;
...
export class Graph {
  ...
  layoutLabels_(minValue: number, maxValue: number) {
    if (maxValue - minValue < 1024) {
      this.layoutLabelsBasic_(
        minValue,
        maxValue,
        TimelineGraphView.MAX_DECIMAL_PRECISION
      );
      return;
    }
  ...
  layoutLabelsBasic_(
    ...
    // gap.
    let minLabelSpacing =
      2 * this.fontHeight_ + TimelineGraphView.LABEL_VERTICAL_SPACING;

    // The + 1 is for the top label.
    let maxLabels = 1 + this.height_ / minLabelSpacing;
    if (maxLabels < 2) {
      maxLabels = 2;
    } else if (maxLabels > TimelineGraphView.MAX_VERTICAL_LABELS) {
      maxLabels = TimelineGraphView.MAX_VERTICAL_LABELS;
    }
  ...
  drawTicks(context: CanvasRenderingContext2D) {
    ...
    let x2;
    x1 = this.width_ - 1;
    x2 = this.width_ - 1 - TimelineGraphView.Y_AXIS_TICK_LENGTH;

    context.fillStyle = TimelineGraphView.GRID_COLOR;
    context.beginPath();
  ...
  drawLabels(context: CanvasRenderingContext2D) {
    ...
    let x = this.width_ - TimelineGraphView.LABEL_HORIZONTAL_SPACING;
